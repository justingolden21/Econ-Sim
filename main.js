...
//upkeepOffset does what? nothing really I think.
      ...
      report(firm);
		...
		// money is last step, and must be
	...
	payUpkeep() {
		if(this.hasUpkeep() ) {
		  ...
		  //can we console.log what this is here, the circumstances
			this.bankrupt = true;
		}
	}
	doProduction() {
		...
		//amountProduced = eventAffect(amountProduced); //where we return the effect of the event on production
		//amountProduced = locationAffect(amountProduced); //where we add the effect of a location on output
	...
	adjust() {
		...
		if(this.prevAmountProduced > this.prevAmountSold) { //if produced more than sold
		// if(this.prevAmountProduced/2 > this.prevAmountSold) { // produced more that 2 * sold
			...
			this.sell[sellResource] += Math.round(factor + 1); //maybe instead of + 1, it should be factor * 10% of price, or if price is 1, factor + 1? Maybe not.
		}
...
function newFirm(firmType, sellPrice=10) {
	...
	if(getFirmCount('farm')>=MAX_LIMIT && firmType == 'farm') // bobby's stupid logic for experiments
...
function tick(overridePause=false) {
	...
	for(let i=0; i<AIs.length; i++) {
		if(AIs[i] && AIs[i].bankrupt) {
			...
			AIs[i] = undefined;
			numBankrupt++;
			console.log("Total Bankrupt: " + numBankrupt);
		}
	}
